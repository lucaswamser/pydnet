...
predict_image.argtypes = [c_void_p, IMAGE, c_float, c_char_p,POINTER(DETECTION)]
predict_image.restype = c_int
...
def classify(network,im,thresh = 0):
   ...
   p = predict_image(network.net, im ,c_float(thresh),network.names,elems)
   out = (DETECTION * p)()	
   for a in range(0,p):
	out[a] = elems[a]
   return out
...
def drawDetections(image,dec):
    draw_detections(image,dec,len(dec))
